...
// skipping this means refund/invoice-creation can never resolve any of them. Exported (not just
// used internally) — agent/dashboard.ts reuses this same scan for cross-customer aggregates
// (e.g. overdue invoices) rather than re-solving the same test-clock quirk a second time.
export async function* listAllCustomers(stripe: Stripe): AsyncGenerator<Stripe.Customer> {
